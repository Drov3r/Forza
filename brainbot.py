...
network = NeuralNetworkTest.NeuralNetwork()
np_load_old = np.load
np.load = lambda *a, **k: np_load_old(*a, allow_pickle=True, **k)
train_data = np.load('training_datav2.npy')
np.load = np_load_old
training_inputs=train_data[0]
training_outputs=train_data[1]
...
while True:
    ...
    if (values[0] > 0):
        ...
        out = network.think(np.array([values[84],accel,brake,turn]))
        out[0]=round(out[0])
        out[1]=round(out[1])
        out[2]=round(out[2])
        out[3]=round(out[3])
